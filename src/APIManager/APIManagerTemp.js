...
export default class APIManagerTemp{
    ...
    static makeGetRequest = async (endpoint, props, callback, showLoader = true) => {
        ...
        await getData(asyncStorageKey.userData,async (userData) => {
            if(userData){
    ...
    static makePostRequest = async (endpoint, dictParam, props, callback, showLoader = true) => {
        ...
        // this.encryptText(JSON.stringify(dictParam), async (data) => {
            ...
            await axios.post(this.getURL(endpoint), JSON.stringify(dictParam), { headers: header, timeout: 60000 }).then((response) => {
                toggleLoader(false)
                if (response.status == 200) {
